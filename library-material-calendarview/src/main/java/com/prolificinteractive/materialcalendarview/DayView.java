...
//        API 22. Technically harmless to leave on for API 21 and 23, but not worth risking for 23+
    ...
    //// TODO: 2017/7/30 修改显示区域大小
    private void calculateBounds(int width, int height) {
//        Log.d("DayView", "width:" + width);
//        Log.d("DayView", "height:" + height);
        ...
        if (width >= height) {
            tempRect.set(offset+20, 0+20, radius + offset, height);
            circleDrawableRect.set(circleOffset+20, 0+20, radius + circleOffset, height);
        } else {
            tempRect.set(0, offset, width, radius + offset);
            circleDrawableRect.set(0+20, circleOffset+20, width, radius + circleOffset);
        }
    }
}
